...
echo "================================================"
echo " BMI Health Tracker — Build & Push (LoadBalancer variant)"
echo " Image tag : ${TAG}"
...
patch_image_tag k8s-LoadBalancer/backend/deployment.yaml  backend  "${ECR_BASE}/bmi-backend:${TAG}"
patch_image_tag k8s-LoadBalancer/frontend/deployment.yaml frontend "${ECR_BASE}/bmi-frontend:${TAG}"
...
if git diff --staged --quiet; then
  ...
  git commit -m "deploy(lb): image tag ${TAG} (${TIMESTAMP})"
