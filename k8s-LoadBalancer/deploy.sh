#!/usr/bin/env bash
# =============================================================================
# deploy.sh (LoadBalancer variant)
# Applies all Kubernetes manifests in the correct dependency order, using
# MetalLB + Service type=LoadBalancer instead of a frontend NodePort.
...
#
# Also manual (not scriptable generically): create an AWS NLB with a target
# group of type "ip" pointing at the MetalLB-assigned EXTERNAL-IP so the app
# is reachable from the public internet. See k8s-LoadBalancer/README.md.
# =============================================================================
...
echo "================================================"
echo " BMI Health Tracker — Kubernetes Deployment (LoadBalancer variant)"
echo "================================================"
echo ""
...
# ── [5/6] MetalLB ──────────────────────────────────────────────────────────────
# Installs MetalLB and its IPAddressPool/L2Advertisement — no
# cloud-controller-manager on this kubeadm cluster, so Service
# type=LoadBalancer would otherwise stay Pending forever.
echo "[5/6] Installing MetalLB..."
...
echo "      Waiting for MetalLB to assign an EXTERNAL-IP (up to 60s)..."
for _ in $(seq 1 12); do
  EXTERNAL_IP=$(kubectl get svc bmi-frontend-svc -n "${NAMESPACE}" \
...
if [[ -n "${EXTERNAL_IP:-}" ]]; then
  echo "MetalLB EXTERNAL-IP: ${EXTERNAL_IP} (VPC-internal only, see README.md)"
  echo "Point an AWS NLB target group (type: ip) at this address for public reachability."
else
  echo "⚠️  No EXTERNAL-IP assigned yet — check: kubectl get svc bmi-frontend-svc -n ${NAMESPACE}"
fi
