...
echo "================================================"
echo " BMI Health Tracker — Build & Push (LoadBalancer-annotations variant)"
echo " Image tag : ${TAG}"
...
patch_image_tag k8s-LoadBalancer-annotations/backend/deployment.yaml  backend  "${ECR_BASE}/bmi-backend:${TAG}"
patch_image_tag k8s-LoadBalancer-annotations/frontend/deployment.yaml frontend "${ECR_BASE}/bmi-frontend:${TAG}"

# Commit and push the updated manifests so the cluster always gets the latest
git add k8s-LoadBalancer-annotations/backend/deployment.yaml k8s-LoadBalancer-annotations/frontend/deployment.yaml
# Only commit if there's actually a change (first run or new tag)
if git diff --staged --quiet; then
  echo "      Manifests unchanged (same git SHA) — no commit needed."
else
  git commit -m "deploy(lb-annotations): image tag ${TAG} (${TIMESTAMP})"
